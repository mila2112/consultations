...
const sendReminders = async (consultation) => {
    try {
        ...
        logger.info(`${reminderDate} | Привет ${client.firstName}. Напоминаем о консультации с юристом ${lawyer.firstName} завтра в ${consultation.startTime}.`);
    } catch (err) {
...
const reminderJob = new cron.CronJob("0 0 * * *", async() => {
    try{
        console.log("start every day")
...
const reminderTwoHoursJob = new cron.CronJob("0 * * * *", async() => {
    try {
        console.log("start every 1 hour");
        const currentDate = moment();
